...
std::vector<std::string> StackWalker::ShowCallstack(HANDLE hThread, CONTEXT *context, PReadProcessMemoryRoutine readMemoryFunction, LPVOID pUserData)
{
...
#error "Platform not supported!"
    ...
    for (frameNum = 0; ; ++frameNum)
    {
        ...
        this->OnCallstackEntry(et, csEntry, lOutput);

        if (s.AddrReturn.Offset == 0)
        {
            this->OnCallstackEntry(CallstackEntryType::lastEntry, csEntry, lOutput);
...
void StackWalker::OnLoadModule(LPCSTR img, LPCSTR mod, DWORD64 baseAddr, DWORD size, DWORD result, LPCSTR symType, LPCSTR pdbName, ULONGLONG fileVersion)
{
    ...
    //HACK OnOutput(buffer);
}

void StackWalker::OnCallstackEntry(CallstackEntryType eType, CallstackEntry &entry, std::vector<std::string>& lOutPut)
{
    CHAR buffer[STACKWALK_MAX_NAMELEN];
...
void StackWalker::OnOutput(LPCSTR buffer, std::vector<std::string>& lOutPut)
{
    lOutPut.push_back(buffer);
    /*OutputDebugStringA(buffer); */ //HACK
}
